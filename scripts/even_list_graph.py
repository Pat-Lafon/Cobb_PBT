...
# cli
parser = argparse.ArgumentParser(description="Generate bar chart for Even List only.")
...
def create_even_list_graph(table_name):
    fig, ax = plt.subplots(1, 1)
    fig.set_size_inches(10, 7)

    # Set labels
    if table == "table1":
        fig.suptitle("Even List Coverage Repaired", fontsize=fontsize)
        ax.set_ylabel("Number of Valid Values", fontsize=fontsize)
    else:
        fig.suptitle("Even List Safety Repaired", fontsize=fontsize)
        ax.set_ylabel("Number of values out of 20k accepted", fontsize=fontsize)

    ax.set_xlabel("Generator variations", fontsize=fontsize)

    # Read data from Even list CSV
    csv_file = "./csv/even_list/" + table + ".csv"
    ...
    try:
        ...
        # Save the graph
        plt.tight_layout()
        output_file = os.path.join(output_dir, f"even_list_{table_name}.png")
        plt.savefig(output_file)
        print(f"Graph saved to: {output_file}")

    except FileNotFoundError:
        print(f"Error: Could not find CSV file: {csv_file}")
        print("Please ensure the Even list data exists in the csv directory.")
    except Exception as e:
        print(f"Error creating graph: {e}")


if __name__ == "__main__":
    create_even_list_graph(table)
