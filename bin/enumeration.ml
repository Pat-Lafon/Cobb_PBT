...
let () = Core.Signal.Expert.handle Core.Signal.alrm (fun (_:Core.Signal.t) ->
     Printf.printf "Timed out"; time_out_ref := true)
...
let default_gen h =
  gen_sized_int_root_red_rbtree (2 * h)

let black_heights : int list = [1; 2; 3; 4; 5; 6]
let number_of_trees : int list = [1000; 10000]
let synthesized_gen_results_file = "bin/enumeration_data/synthesis_results.csv.results"
let default_gen_results_file = "bin/enumeration_data/default_results.csv.results"
...
let () = List.iter (fun (filename, gen) ->
...
let () = List.iter (fun h ->
  List.iter (fun n ->
    ...
    if gen == default_gen then print_endline "hello";
      let rbtree_prog_time : float = run_X_times (fun () -> gen h, h) (Some (fun (t, h) -> rbtree_invariant t h)) (fun (x, _) -> count_rbtree_nodes x) n
      in
      output_string oc (Printf.sprintf "%d,%d,%f\n" h n rbtree_prog_time)
    with Timed_out ->
      print_endline "Caught timed out"
  ) number_of_trees
) black_heights in

close_out oc) benchmarks