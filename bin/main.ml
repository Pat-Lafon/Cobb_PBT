...
let is_unique_prog1_safe l s = has_same_size l s && is_unique l
let is_unique_prog2_safe l s = has_same_size l s && is_unique l
let is_unique_prog3_safe l s = has_same_size l s && is_unique l
...
let is_sorted_prog1_safe l s x = has_same_size l s && has_duplicate_int_x l x
let is_sorted_prog2_safe l s x = has_same_size l s && has_duplicate_int_x l x
let is_sorted_prog3_safe l s x = has_same_size l s && has_duplicate_int_x l x

(* QCheck.make *)
let precondition_frequency prop (gen_type, name) =
  QCheck.(
    Test.make ~count:test_count ~max_fail:test_max_fail ~name gen_type (fun l ->
        assume (prop l);
        func l))
...
let create_test_pair_list prop gens =
  List.map (precondition_frequency_pair prop) gens
...
let pair_prop_assoc_list = [ ("is_sized", is_sized) ]
let option_prop_assoc_list = [ ("is_sorted", is_sorted) ]
...
let () =
  ...
  try
    ...
    let tests =
      ...
      | Single g ->
          create_test_list (List.assoc prop_name single_prop_assoc_list) g
      | Pair g ->
          create_test_pair_list (List.assoc prop_name pair_prop_assoc_list) g
      | Option g ->
          create_test_list (List.assoc prop_name option_prop_assoc_list) g
    in
  ...
  (* prints usage *)
  | Invalid_argument s ->
    print_endline "Usage: dune exec Cobb_PBT -- property_name generator_name"
